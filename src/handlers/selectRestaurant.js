...
export default async context => {
  ...
  let categoryId = Categories.map(function(category) {
    let excluded = context.state.excludedCategories.find(excludedCategories => {
  ...
  const params = {
    categoryId: context.state.excludedCategories.length ? categoryId : process.env.REACT_APP_FOURSQUARE_FOOD_CATEGORY,
  ...
  console.log(`${process.env.REACT_APP_FOURSQUARE_BASE_URL}venues/search?${Query.stringify({ ...params, ...credentials })}`);
